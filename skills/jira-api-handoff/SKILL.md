...
description: Jira 이슈에 ADF 댓글을 작성/갱신한다. 세 모드 — 백엔드 API 변경을 프론트에 전달(프론트 핸드오프), 버그 수정 완료를 QA에 전달(QA 핸드오프), 버그가 아닌 건의 원인·정책을 회신(원인 회신). 신규/필드추가 색상 뱃지·Swagger 딥링크(프론트), 원인/수정/QA확인 3블록 + MR 링크 푸터(QA), 표·불릿·뱃지 없는 평문 문단(원인 회신). 게시 전 항상 사용자 확인을 받는다. "API 변경 프론트한테 넘겨줘", "Jira에 백엔드 수정사항 댓글", "핸드오프 댓글", "QA 넘겨줘", "수정완료 댓글", "버그 아니라고 댓글", "정책 문의 답변", "원인 설명 댓글" 등에 사용.
argument-hint: "[Jira URL 또는 이슈키] [--qa | --front | --info] (선택: 변경 API 설명 · 수정 내용 · 원인 요지)"
allowed-tools:
  - Read
  - Glob
  - Grep
...
1. **프론트 API 핸드오프**: 백엔드 API 변경(신규 API / 기존 응답 필드 추가)을 프론트 개발자가 보기 좋게 Jira 이슈 댓글로 정리. 색상 상태 뱃지·필드별 타입·설명·Swagger 딥링크를 갖춘 ADF 댓글.
2. **QA 핸드오프**: 버그 수정 완료를 QA 담당자에게 알리는 댓글. 원인/수정/QA확인 3블록 구조 + `[수정 완료]` 상태 머리말. 배포 여부는 평문으로 밝히고, MR 링크는 맨 마지막 푸터 한 줄로 분리한다.
3. **원인 회신** (`--info`): **버그가 아닌 건**에 회신하는 댓글. 정책 문의, 사양대로 동작하는 QA 리포트, 테스트 데이터·환경 예외 등. 표·불릿·뱃지·구분선 없이 짧은 평문 문단으로만 쓴다. 고칠 것이 없다는 결론을 읽는 사람이 한 번 읽고 이해할 수 있게 만드는 것이 목적이다.

항상 한국어로 응답한다.

**모든 모드 공통: 게시 전 사용자 확인이 필수다.** 아래 "게시 전 확인" 절차를 거치지 않고 `addCommentToJiraIssue`를 호출하지 않는다.
...
- ARGS에 `--info`, 또는 "버그 아니", "정상 동작", "사양대로", "정책 문의", "원인 설명", "테스트 데이터" 등 **고칠 코드가 없다는 맥락** → 원인 회신 모드.
- 대화 맥락에서 이미 "코드 결함 없음"으로 결론이 난 조사 뒤에 "댓글 남겨줘"가 오면 → 원인 회신 모드.
- 판별 불가 시 AskUserQuestion:
  > 어떤 모드로 댓글을 작성할까요?
  > 1. 프론트 API 핸드오프 — API 변경사항을 프론트 개발자에게 전달
  > 2. QA 핸드오프 — 버그 수정 완료를 QA 담당자에게 전달
  > 3. 원인 회신 — 버그가 아닌 건의 원인·정책을 설명

모드 선택 후 Step 1로 진행.

> 수정 커밋이 없는데 QA 모드를 쓰지 않는다. `[수정 완료]` 뱃지와 MR 푸터는 고친 것이 있을 때만 의미가 있다. 고칠 게 없는 건은 원인 회신 모드다.

---

### Step 1: Jira 이슈 해석 (모드 공통)
...
- **먼저 "게시 전 확인"을 거친다.** 승인 없이 게시하지 않는다.
...
버그가 아닌 건에 회신할 때 쓴다. 정책 문의 답변, 사양대로 동작하는 QA 리포트, 테스트 데이터·환경 때문에 발생한 예외.

**목적은 읽는 사람이 리소스를 들이지 않는 것이다.** 독자는 QA·기획이고, 이 댓글을 한 번 읽고 "아 그래서 문제가 아니구나"까지 도달해야 한다. 조사 과정·검증 근거를 다 옮기면 실패한다. 결론과 그 결론이 나온 이유만 남긴다.

### Step I-2: 정보 수집

- **판정**: 사양대로 동작 / 테스트 데이터 예외 / 환경 예외 / 정책 오해 중 무엇인지 하나로 정한다.
- **기준**: 관련 정책과 설정값. 비개발자 언어로 쓴다(약정 회차, 가능 횟수, 기준 기간 등).
- **원인**: 왜 문제처럼 보였는지. 한 문단.
- **근거**: 실제 데이터 값. 날짜·건수·구간을 구체적인 숫자로 그대로 인용한다.
- **개선 제안**(선택): 재발 방지나 QA 편의 개선. `ps.`로 붙인다.
- **별도 후속**(선택): 문구 수정처럼 다른 이슈로 넘길 것이 있으면 한 줄.
- **멘션 대상**: `getJiraIssue(fields:["reporter"])`로 reporter의 `accountId`와 표시이름을 얻는다. 멘션은 추측한 accountId로 만들지 않는다.

코드 경로·클래스명·메서드명·컬럼명·enum 값을 넣지 않는다. 이 모드의 독자는 코드를 읽지 않는다.

### Step I-3: 본문 구조 (평문 문단)

```
doc
├─ paragraph: mention(@담당자)
├─ paragraph: "안녕하세요 {호칭}!"
├─ paragraph: 기준 — 관련 정책·설정값
├─ paragraph: 원인 — "이슈가 발생하는 이유는 ~"
├─ paragraph: 근거 — 실제 데이터 값(날짜·건수·구간)
├─ paragraph: 판정 — "요약하자면 ~로 판단됩니다."
├─ [선택] paragraph: "ps. ~ 방안을 고려해야 할 것 같습니다."
├─ paragraph: "궁금한사항 있으시면 메신져 주시면 설명드리겠습니다."
└─ paragraph: "감사합니다."
```

문단은 6~9개, 각 문단 1~3문장. 전체가 스크롤 없이 읽히는 길이를 넘기지 않는다.

**멘션 노드**
```json
{"type":"paragraph","content":[
  {"type":"mention","attrs":{"id":"{reporter accountId}","text":"@{표시이름}"}}]}
```

**본문 문단(마크 없는 평문)**
```json
{"type":"paragraph","content":[
  {"type":"text","text":"이슈가 발생하는 이유는 ~ 때문입니다."}]}
```

### 이 모드의 금지 목록

QA·프론트 모드에서 쓰는 시각 장치를 여기서는 쓰지 않는다.

- 표, 불릿 목록, 구분선(rule), status 뱃지
- 굵은글씨 강조(`strong`), 화살표(`→`), 코드체(`code`)
- `원인:` `수정:` 같은 라벨 나열
- "X가 아니라 Y" 대조 구문, 굵은글씨 슬로건

라벨과 뱃지는 수정 보고에는 맞지만 설명 회신에서는 훑어읽게 만들어서 정작 내용이 전달되지 않는다. 평문 문단으로만 쓴다.

### 톤

- 인사와 맺음을 갖춘 공손한 존댓말. 사내 메신저에 쓰는 정도의 평상어를 유지한다.
- 판정을 흐리지 않는다. "확인이 필요합니다"로 끝내지 말고 "~로 판단됩니다"까지 쓴다.
- QA가 리포트를 잘못 썼다는 식의 표현을 쓰지 않는다. 데이터·환경 조건을 사실로 기술한다.
- 개선 제안은 지시가 아니라 제안으로 쓴다("~ 방안을 고려해야 할 것 같습니다").

### 참조 예시 (이 모드의 기준)

실제로 발행한 댓글이다. 문단 구성·길이·톤을 이 수준으로 맞춘다.
...
문의주신 정기구독의 약정은 59번 약정은 6회차 이며, 약정 기간내 건너뛰기 2회가능,
약정기간 종료후 1년기간내 건너뛰기 4회 가능입니다.

이슈가 발생하는 이유는 수동 스케줄러 결제로 약정을 종료 시키는 과정에서,
약정종료일(6회차결제일)이 "2026-07-29" 이 되었고, 이때부터 1년 기간이 카운팅되기 때문에
1년기준이 "2026-07-29~2027-07-28" 으로 카운팅됩니다.

건너뛰기 4건(2026-08-29, 2026-10-29, 2027-04-29, 2027-05-29)이 전부 이 기간안에 포함되어
건너뛰기가 소진된 것으로 판단됩니다.

요약하자면 정기구독 스케줄러를 수동으로 실행시켜 발생한 테스트서버 예외로 판단됩니다.

ps.추후 이런 케이스의 원활한 QA를 위해서는 스케줄러 실행시 스케줄러에 입력시킨 결제 날짜로
모든 데이터의 입력 날짜가 치환되도록 수정해는 방안을 고려해야할것 같습니다.

궁금한사항 있으시면 메신져 주시면 설명드리겠습니다.
...
- 기준(정책·설정값)을 먼저 놓고 원인을 뒤에 둔다. 기준을 모르면 원인이 이해되지 않는다.
- 근거는 실제 값을 괄호에 그대로 나열한다. 조사 과정은 쓰지 않는다.
- 판정을 "~로 판단됩니다" 한 문장으로 못박는다.
- 개선 제안은 `ps.`로 본문과 분리한다.
- 조사하면서 알게 된 부수적인 발견(다른 잠재 버그 등)은 이 댓글에 넣지 않는다. 별도 이슈로 분리한다.

### Step I-4: 게시

아래 "게시 전 확인"을 거친 뒤 `addCommentToJiraIssue(cloudId, issueIdOrKey, contentFormat:"adf", commentBody)`.

**상태 전환·담당자 재할당을 하지 않는다.** 이 모드는 수정 보고가 아니므로 "해결" 전환 대상이 아니다. 사용자가 명시적으로 요청하면 그때만 진행한다.

---

## 게시 전 확인 (모든 모드 필수)

댓글을 게시하기 전에 **반드시** AskUserQuestion으로 승인을 받는다. 승인 없이 `addCommentToJiraIssue`를 호출하지 않는다. 신규 작성과 `commentId` 갱신 모두 해당한다.

1. 완성된 본문을 **평문으로 렌더링해서** 사용자에게 보여준다. ADF JSON을 그대로 보여주지 않는다. 멘션은 `@표시이름`, 링크는 `텍스트(URL)` 형태로 옮긴다.
2. 어느 이슈에 다는지(이슈키·제목)와 어떤 모드인지 한 줄로 함께 밝힌다.
3. AskUserQuestion으로 묻는다.
   > 이 내용으로 게시할까요?
   > 1. 게시 (권장)
   > 2. 수정 후 다시 확인 — 고칠 부분을 알려주시면 반영해서 다시 보여드립니다
   > 3. 취소 — 게시하지 않습니다
4. "게시"를 선택했을 때만 API를 호출한다. "수정"이면 피드백을 반영해 1번부터 반복한다. "취소"면 본문만 남기고 종료한다.

Jira 댓글은 담당자에게 알림이 가는 외부 노출이고 되돌리는 비용이 크다. 사용자가 "바로 올려"라고 미리 말한 경우에도 본문은 한 번 보여주고 확인을 받는다.
...
- **이모지 금지.** 프론트·QA 모드의 시각 강조는 상태 뱃지(색상)와 `→`, 구분선, 코드체로 처리한다. 원인 회신 모드는 시각 장치를 쓰지 않는다.
- 마무리 인사:
  - 프론트 모드: "확인 후 문의사항 있으시면 편하게 말씀 부탁드립니다."
  - QA 모드: "QA 확인 부탁드립니다."
  - 원인 회신 모드: "궁금한사항 있으시면 메신져 주시면 설명드리겠습니다." + "감사합니다."

### 내부정보 포함 여부 (모드별 분기)

| 항목 | 프론트 API 모드 | QA 모드 | 원인 회신 모드 |
|------|----------------|---------|---------------|
| MR 번호 | **제외** (내부 정보) | **맨 마지막 푸터에만** (하이퍼링크) | **제외** (고친 것이 없음) |
| Jenkins 잡명·빌드번호 | **제외** (내부 정보) | **제외** | **제외** |
| "배포 완료" 문구 | **제외** | **포함 필수** (평문, 수정 블록) | **제외** |
| Swagger 딥링크 | **포함 필수** | 불필요(생략) | 불필요(생략) |
| 코드 경로·클래스·컬럼명 | **제외** | **제외** | **제외** |
| 실제 데이터 값(날짜·건수) | 불필요 | 필요 시 | **포함 필수** (판정 근거) |
...
- 운영 이슈에 댓글을 다는 것은 외부 노출 행위다. **본문을 사용자에게 보여주고 AskUserQuestion으로 승인받은 뒤에만 게시한다**("게시 전 확인" 절 참조). 이 단계를 건너뛰지 않는다.
- 원인 회신 모드에 표·불릿·뱃지를 넣지 않는다. 정보를 더 담으려다 읽는 부담을 늘리면 이 모드의 목적을 잃는다.
